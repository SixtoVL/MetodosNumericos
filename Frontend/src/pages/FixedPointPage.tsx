...
import { FixedPointForm } from '../components/forms/FixedPointForm';
  ...
  return (
    <div className={styles.pageContainer}>
      <header className={styles.header}>
        ...
        <p>Resolución de ecuaciones y sistemas mediante la iteración funcional x = g(x).</p>
      </header>
        ...
        <aside>
          ...
          {isPending && (
            <div style={{ textAlign: 'center', padding: '2rem', background: 'white', borderRadius: '12px', marginTop: '1rem', boxShadow: '0 4px 6px -1px rgb(0 0 0 / 0.1)' }}>
              ...
              <p style={{ marginTop: '1rem', color: '#64748b', fontWeight: 500 }}>Iterando funciones...</p>
            </div>
          )}
        ...
        <main className={styles.mainContent}>
          {data ? (
            <>
              ...
              {/* Cuadro explicativo enriquecido */}
              <FormulaDisplay formulas={{
                funciones: data.formulas.g_latex.map((f, i) => `x_{${i+1}} = ${f}`),
                jacobiano: `${data.formulas.metodo} \\text{ (Desplazamientos Simultaneos)}`
              }} />
            ...
            <div className={styles.emptyState}>
              ...
              <div style={{ textAlign: 'center' }}>
                ...
                <p style={{ fontSize: '0.95rem' }}>Ingrese las funciones g(x) y el punto inicial para comenzar.</p>
              </div>
            </div>
          )}
        </main>
